...
export function AnimatedClue({hintRevision,hintOpened,onStarted,onError}:AnimatedClueProps){
 ...
 useEffect(()=>{sound.current=createSceneSound(()=>setSoundError(true));return()=>{sound.current?.dispose();sound.current=null}},[])
 ...
 useEffect(()=>{
  ...
  import('../cinema-scene.mjs').then(({makeKitchen})=>{
   ...
   const tick=(now:number)=>{if(cancelled)return;const dt=Math.min((now-last)/1000,.06);last=now;const c=controls.current
    if(!document.hidden){if(c.playing){c.time=Math.min(22,c.time+dt);if(c.time===22){c.playing=false;c.audioActive=false;setPlaying(false)}}
 ...
 function inspect(){sound.current?.unlock();const c=controls.current;c.started=true;c.time=22;c.view='inspect';c.playing=false;c.audioActive=true;c.instant=reduced.current;setStarted(true);setPlaying(true);setAway(true);setTime(22);syncSound()}
 ...
 return <div className={`logic-animation${expanded?' is-expanded':''}`} role={expanded?'dialog':undefined} aria-modal={expanded||undefined} aria-label="Before Service animated clue">
  ...
  <div className="logic-film-controls"><button disabled={!ready} onClick={()=>{const c=controls.current;if(!started||c.time===22&&c.view==='auto'){begin();return}if(c.view==='auto')c.audioActive=c.playing=!c.playing;else c.audioActive=!c.audioActive;if(c.audioActive)sound.current?.unlock();setPlaying(c.audioActive);syncSound()}}>{playing?'Pause':time===22&&controls.current.view==='auto'?'Replay':'Play'}</button>
   <button disabled={!ready||!started} onClick={()=>{const c=controls.current;c.playing=false;c.audioActive=false;c.view='auto';c.time=Math.min(22,c.time+3);c.instant=true;setTime(c.time);setPlaying(false);setAway(false);syncSound()}}>Next moment</button>
   <button disabled={!ready||!started} onClick={()=>{const c=controls.current;sound.current?.unlock();c.playing=false;c.audioActive=true;c.time=22;c.view=c.view==='away'||c.view==='inspect'?'watch':'away';c.instant=reduced.current;setTime(22);setPlaying(true);setAway(c.view==='away');syncSound()}}>{away?'Look back':'Look away'}</button>
