...
export function AnimatedClue({hintRevision,hintOpened,onStarted,onError}:AnimatedClueProps){
 ...
 const [media,setMedia]=useState(()=>{try{return readMediaPreferences(localStorage.getItem(MEDIA_KEY))}catch{return readMediaPreferences(null)}})
 ...
 useEffect(()=>{
  ...
  import('../cinema-scene.mjs').then(({makeKitchen})=>{
   ...
   const tick=(now:number)=>{if(cancelled)return;const dt=Math.min((now-last)/1000,.06);last=now;const c=controls.current
    if(!document.hidden){if(c.playing){c.time=Math.min(22,c.time+dt);if(c.time===22){c.playing=false;setPlaying(false)}narrate()}
 ...
 return <div className={`logic-animation${expanded?' is-expanded':''}`} role={expanded?'dialog':undefined} aria-modal={expanded||undefined} aria-label="Before Service animated clue">
  ...
  <div className="logic-media-options" role="group" aria-label="Listening and subtitles">
   <button type="button" aria-pressed={media.voice&&speechAvailable} disabled={!speechAvailable} onClick={toggleVoice}>Voice-over: {media.voice&&speechAvailable?'On':'Off'}</button>
   <button type="button" aria-pressed={media.subtitles} onClick={()=>setMedia(p=>({...p,subtitles:!p.subtitles}))}>Subtitles: {media.subtitles?'On':'Off'}</button>
  </div>
  {(!speechAvailable||voiceError)&&<p className="logic-voice-notice" role="status">{voiceError||'Voice-over is not supported in this browser. You can still use subtitles.'}</p>}
