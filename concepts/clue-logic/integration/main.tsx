...
function Prototype(){
 ...
 <section className="case-home-card"><p className="kicker">NEW · A FIVE-PUZZLE MYSTERY</p><h2>The opening-night mix-up</h2><p>Solve five phrases. Collect five discoveries. Work out who changed the soup—and what they added.</p><Button onClick={()=>setMystery(true)}>Open the case →</Button></section><section className="logic-scene-list" aria-label="Playtest scenes">{playableScenes.map((entry,index)=><article key={entry.id}><span className="eyebrow">{index>1?'NEW · ':''}SCENE {String(index+1).padStart(2,'0')} · {entry.place.toUpperCase()}</span><h2>{sceneTitles[index]}</h2><p>{entry.lead}</p><Button variant="secondary" onClick={()=>setScene(index)}>Play scene {index+1} →</Button></article>)}</section>
