...
export function restoreActions(raw:string|null):{actions:Action[];round:Round;feedback?:DifficultyFeedback}{
 ...
 try{const data=JSON.parse(raw||'null');if(data?.version!==1||!Array.isArray(data.actions))return result
  for(const a of data.actions.slice(0,500)){if(!a||!['letter','phrase','hint'].includes(a.type))continue;if(a.type==='letter'&&typeof a.letter!=='string'||a.type==='phrase'&&typeof a.answer!=='string'||a.type==='hint'&&a.id!=='eyes')continue;result.round=act(scene,result.round,a).round as Round;result.actions.push(a)}
