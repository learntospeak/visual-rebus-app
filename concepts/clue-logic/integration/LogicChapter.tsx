...
export function LogicChapter({initialScene=0,...props}:LogicChapterProps){const [index,setIndex]=useState(initialScene===1?1:0);return <LogicRound key={index} {...props} index={index} onNext={()=>index===0?setIndex(1):props.onExit()}/>}
function LogicRound({onExit,onComplete,storageKey=SAVE_KEY,index,onNext}:LogicChapterProps&{index:number;onNext:()=>void}){
 ...
 if(solved)return <SolvedScreen puzzle={puzzle} outcome={{revealed:false,stars:round.lives,cluesUsed:round.hints.length,seconds:Math.round((Date.now()-began.current)/1000),daily:false}} isLastPuzzle={index===1} onHome={onExit} onNext={onNext} showReminderOffer={false} onEnableReminder={async()=>false} onDismissReminder={()=>{}} difficultyFeedback={state.feedback} onDifficultyFeedback={feedback=>setState(s=>({...s,feedback}))}/>
 return <><PuzzleScreen puzzle={puzzle} puzzleNumber={index+1} puzzleCount={2} sceneTitle={sceneTitles[index]} guess={guess} clueCount={round.hints.length} message={storageFailed?'Progress cannot be saved in this browser. '+message:round.status==='won'?'Previously solved. Replay the animation or return home.':message} lockedLetters={locked} celebrating={celebrating} onHome={onExit} onGuessChange={setGuess} onSubmit={submit} onClue={hint} onReveal={()=>{}} onInteractionSolved={()=>{}} lives={round.lives} blocked={round.status==='lost'} completed={round.status==='won'} misses={round.misses} clueDisabled={!started||round.lives<=1||round.hints.length>0||round.status!=='playing'} onRetry={retry} visual={<AnimatedClue key={attempt} sceneIndex={index} hintRevision={hintRevision} hintOpened={round.hints.length>0} onStarted={()=>setStarted(true)} onError={setMessage}/>}/>
 {round.status==='won'&&!celebrating&&<div className="logic-replay-solved"><button className="secondary-button" onClick={()=>setSolved(true)}>{index===0?'View result / next scene':'View result'}</button><button className="text-button" onClick={retry}>New attempt</button></div>}</>
}
