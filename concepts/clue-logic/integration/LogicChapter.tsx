...
export interface LogicChapterProps {onExit:()=>void;initialScene?:number;onComplete?:(result:Completion)=>void;storageKey?:string;singleScene?:boolean;onChapterComplete?:()=>void;puzzleOrdinal?:number;puzzleTotal?:number;sceneOrder?:number[];lastInGroup?:boolean}
export function LogicChapter({initialScene=0,...props}:LogicChapterProps){
 const order=props.sceneOrder??puzzles.map((_,i)=>i)
 const [index,setIndex]=useState(order.includes(initialScene)?initialScene:order[0])
 const position=order.indexOf(index)
 return <LogicRound key={index} {...props} index={index} puzzleOrdinal={props.puzzleOrdinal??position+1} puzzleTotal={props.puzzleTotal??order.length} lastInGroup={position===order.length-1} onNext={()=>props.singleScene?props.onChapterComplete?.():position<order.length-1?setIndex(order[position+1]):props.onExit()}/>
}
function LogicRound({onExit,onComplete,storageKey=SAVE_KEY,index,onNext,singleScene,puzzleOrdinal,puzzleTotal,lastInGroup}:LogicChapterProps&{index:number;onNext:()=>void}){
 ...
 if(solved&&singleScene)return <main className="app-shell case-shell case-puzzle-solved"><p className="kicker">PHRASE SOLVED · EVIDENCE SECURED</p><h1>{puzzle.answer}</h1><p>{round.lives} of 3 lifelines remaining.</p><p>There is something here that could help explain what happened to the soup.</p><Button onClick={onNext}>Inspect the evidence →</Button><Button variant="secondary" onClick={onExit}>Case overview</Button></main>
 if(solved)return <SolvedScreen puzzle={puzzle} outcome={{revealed:false,stars:round.lives,cluesUsed:round.hints.length,seconds:Math.round((Date.now()-began.current)/1000),daily:false}} isLastPuzzle={lastInGroup??index===puzzles.length-1} onHome={onExit} onNext={onNext} showReminderOffer={false} onEnableReminder={async()=>false} onDismissReminder={()=>{}} difficultyFeedback={state.feedback} onDifficultyFeedback={feedback=>setState(s=>({...s,feedback}))}/>
 return <><PuzzleScreen puzzle={puzzle} puzzleNumber={puzzleOrdinal??index+1} puzzleCount={puzzleTotal??puzzles.length} sceneTitle={sceneTitles[index]} guess={guess} clueCount={round.hints.length} message={storageFailed?'Progress cannot be saved in this browser. '+message:round.status==='won'?'Previously solved. Replay the animation or return home.':message} lockedLetters={locked} celebrating={celebrating} onHome={onExit} onGuessChange={setGuess} onSubmit={submit} onClue={hint} onReveal={()=>{}} onInteractionSolved={()=>{}} lives={round.lives} blocked={round.status==='lost'} completed={round.status==='won'} misses={round.misses} clueDisabled={!started||round.lives<=1||round.hints.length>0||round.status!=='playing'} onRetry={retry} visual={<AnimatedClue key={attempt} sceneIndex={index} hintRevision={hintRevision} hintOpened={round.hints.length>0} onStarted={()=>setStarted(true)} onError={setMessage}/>}/>
