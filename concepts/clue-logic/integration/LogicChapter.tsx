...
export interface LogicChapterProps {onExit:()=>void;initialScene?:number;onComplete?:(result:Completion)=>void;storageKey?:string;singleScene?:boolean;onChapterComplete?:()=>void;puzzleOrdinal?:number;puzzleTotal?:number}
export function LogicChapter({initialScene=0,...props}:LogicChapterProps){const [index,setIndex]=useState(Math.max(0,Math.min(puzzles.length-1,Math.floor(initialScene)||0)));return <LogicRound key={index} {...props} index={index} onNext={()=>props.singleScene?props.onChapterComplete?.():index<puzzles.length-1?setIndex(index+1):props.onExit()}/>}
function LogicRound({onExit,onComplete,storageKey=SAVE_KEY,index,onNext,singleScene,puzzleOrdinal,puzzleTotal}:LogicChapterProps&{index:number;onNext:()=>void}){
