...
const canonicalPremiumGeneratedPuzzleIds = [408, 413, 429, 431, 433, 437, 439, 441, 443, 444, 445, 446, 447, 449, 451, 452, 453]
const canonicalPremiumInteractivePuzzleIds = [408, 431, 433, 441, 453]
const canonicalPremiumRevisionIds = [350, 408, 431, 433, 439, 441, 444, 445, 446, 449]
const reworkedVectorPuzzleIds = [
  ...
  247, 248, 249, 250, 251, 252, 253, 254, 255, 257, 258, 260, 261, 271, 272, 273, 274, 275, 276, 277, 278, 279, 280, 281, 282, 283, 284, 285, 286, 287, 288, 289, 290, 291, 292, 293, 294, 295, 296, 297, 298, 299, 300, 301, 302, 303, 304, 305, 306, 307, 308, 309, 310, 328, 337, 357, 392, 400, 435,
  477, 497, 554,
]
const reviewedStyledPuzzleIds = [
  226, 230, 232, 244, 245, 274, 275, 301, 302, 303, 304, 305, 322, 324, 326, 327, 330,
  332, 341, 346, 353, 354, 358, 391, 398, 406, 415, 425, 440, 462, 465, 475, 478, 488,
  496, 498, 500, 504, 506, 508, 510, 520, 549, 560, 565,
]
...
function migrateStarterPuzzle(draft: StarterPuzzleDraft): Puzzle {
  ...
  return {
    ...
    contentVersion: `p${String(draft.id).padStart(3, '0')}-v${draft.id === 253 ? 9 : draft.id === 441 ? 8 : draft.id === 252 ? 4 : [118, 119, 121].includes(draft.id) || canonicalPremiumRevisionIds.includes(draft.id) ? 3 : [13, 122, 124, 125].includes(draft.id) || reworkedGeneratedPuzzleIds.includes(draft.id) || reworkedVectorPuzzleIds.includes(draft.id) || reviewedStyledPuzzleIds.includes(draft.id) || canonicalPremiumBatchIds.includes(draft.id) ? 2 : 1}`,
    chapterId,
    chapterOrder,
    ...
    artwork: {
      version: draft.id === 441 ? 8 : [252, 253].includes(draft.id) ? 4 : [118, 119, 121].includes(draft.id) || canonicalPremiumRevisionIds.includes(draft.id) ? 3 : [13, 122, 124, 125].includes(draft.id) || reworkedGeneratedPuzzleIds.includes(draft.id) || reworkedVectorPuzzleIds.includes(draft.id) || reviewedStyledPuzzleIds.includes(draft.id) || canonicalPremiumBatchIds.includes(draft.id) ? 2 : 1,
...
const chapterSixSpecs: ChapterFiveSpec[] = [
  ...
  { answer: 'blue blood', pattern: '4 5', visual: 'BLOOD', description: 'A single blood drop and the word BLOOD are rendered in unmistakable royal blue', difficulty: 'Hard' },
  ...
  { answer: 'bookworm', pattern: '8', visual: 'BOO 〰●〰 K', description: 'A small worm tunnels physically through the middle of BOOK', difficulty: 'Hard' },
  { answer: 'page turner', pattern: '4 6', visual: 'PAGE  ↻', description: 'PAGE visibly turning', difficulty: 'Hard' },
  { answer: 'chapter and verse', pattern: '7 3 5', visual: 'CHAPTER Ⅰ\n─────────\nVERSE  1–4', description: 'A numbered chapter and its numbered verses appear together on a book page', difficulty: 'Hard' },
  ...
  { answer: 'dead end', pattern: '4 3', visual: 'ROAD ━━━━━━┫ ☠', description: 'A road terminates abruptly at a skull-marked barrier', difficulty: 'Hard' },
  { answer: 'speak of the devil', pattern: '5 2 3 5', visual: 'SPEAK  )))   ♠', description: 'Spoken sound unexpectedly summons a small horned shadow', difficulty: 'Hard' },
  ...
  { answer: 'packed like sardines', pattern: '6 4 8', visual: '┌────────────┐\n│🐟🐟🐟🐟🐟│\n│🐟🐟🐟🐟🐟│\n└────────────┘', description: 'Ten fish are squeezed tightly together inside one narrow tin', difficulty: 'Hard' },
  { answer: 'bigger fish to fry', pattern: '6 4 2 3', visual: '  🐟\n        🐋  →  🍳', description: 'A small fish is ignored while a much larger one heads toward a frying pan', difficulty: 'Hard' },
  ...
  { answer: 'lost for words', pattern: '4 3 5', visual: 'W   O   R   D\n      MAZE 4', description: 'Four separate letter tiles spelling WORD are lost in different dead ends of a hedge maze marked with the number four', format: 'illustration', difficulty: 'Hard' },
  ...
  { answer: 'dark horse', pattern: '4 5', visual: '♞', description: 'A single horse silhouette is almost concealed in darkness', difficulty: 'Hard' },
  { answer: 'like a bull at a gate', pattern: '4 1 4 2 1 4', visual: '🐂  ≫≫≫  ║▦▦▦║', description: 'A charging bull bears down on a solid closed gate', difficulty: 'Hard' },
  { answer: 'lock horns', pattern: '4 5', visual: '⌒╲ LOCK ╱⌒\n  ╲╳╱', description: 'Two curved horns interlock tightly around a lock', difficulty: 'Hard' },
  { answer: 'king of the jungle', pattern: '4 2 3 6', visual: '🌴   👑   🌴\n   🦁\n🌿  🌿  🌿', description: 'A crowned lion sits on a throne of dense jungle foliage', difficulty: 'Hard' },
  { answer: 'make a beeline', pattern: '4 1 7', visual: '🐝 · · · · · · · ▶', description: 'A bee follows one perfectly straight flight path', difficulty: 'Hard' },
...
const chapterSevenSpecs: ChapterFiveSpec[] = [
  ...
  { answer: 'worth your weight in gold', pattern: '5 4 6 2 4', visual: '      ⚖\nWORTH     ●●●\n          GOLD', description: 'WORTH balances exactly against a heavy pile of gold', difficulty: 'Hard' },
  { answer: 'heart of gold', pattern: '5 2 4', visual: 'G O L ♥ D', description: 'A heart forms the glowing centre of GOLD', difficulty: 'Hard' },
  { answer: 'golden opportunity', pattern: '6 11', visual: '╔══      ══╗\n║    ★     ║\n╚══  ⌛  ══╝', description: 'A golden doorway briefly opens onto a prize while an hourglass runs', difficulty: 'Hard' },
  { answer: 'strike gold', pattern: '6 4', visual: 'STRIKE  ⛏  GOLD', description: 'STRIKE drives a pickaxe directly into GOLD', difficulty: 'Hard' },
  { answer: 'roll out the red carpet', pattern: '4 3 3 3 6', visual: '◉════════════\n  ╲__________╱', description: 'A rich red carpet visibly unrolls toward the viewer', difficulty: 'Hard' },
  { answer: 'paint the town red', pattern: '5 3 4 3', visual: '🖌  T O W N\n   ╲████╱', description: 'A paintbrush sweeps a fresh red stroke across TOWN', difficulty: 'Hard' },
  ...
  { answer: 'out of the frying pan into the fire', pattern: '3 2 3 6 3 4 3 4', visual: '   🍳\n    ↗  ●  ↘\n          🔥', description: 'Something leaps out of a frying pan only to land in a larger fire', difficulty: 'Hard' },
  ...
  { answer: 'bite off more than you can chew', pattern: '4 3 4 4 3 3 4', visual: 'BITE  ◖██████████\n             chew', description: 'One bite removes a piece far larger than tiny chew can manage', difficulty: 'Hard' },
  ...
  { answer: 'against all odds', pattern: '7 3 4', visual: '1  3  5  AGAINST  7  9', description: 'A lone person pushes against a wall built entirely from odd-numbered blocks', difficulty: 'Hard' },
  ...
  { answer: 'perfect ten', pattern: '7 3', visual: '1  2  3  4  5\n6  7  8  9  ⑩', description: 'A flawless illuminated score of 10.0 receives the highest judging result', difficulty: 'Hard' },
  { answer: 'two sides of the same coin', pattern: '3 5 2 3 4 4', visual: 'HEADS  ◐  TAILS\n       SAME', description: 'Two contrasting halves are visibly joined as one coin', difficulty: 'Hard' },
  ...
  { answer: 'cost an arm and a leg', pattern: '4 2 3 3 1 3', visual: 'COST  =  💪  +  🦵', description: 'A prosthetic arm and leg sit at a checkout displaying an enormous price', difficulty: 'Hard' },
  ...
  { answer: 'shoulder to shoulder', pattern: '8 2 8', visual: '◯╲SHOULDER│SHOULDER╱◯', description: 'Two figures stand with their shoulders touching', difficulty: 'Hard' },
  ...
  { answer: 'crossed fingers', pattern: '7 7', visual: '╲ ╱\n ╳\n╱ ╲', description: 'Two clearly finger-shaped strokes cross at the centre', difficulty: 'Hard' },
  { answer: 'rule of thumb', pattern: '4 2 5', visual: '0  1  2  3  4\n└──────────┘ 👍', description: 'A measuring rule is calibrated directly against a thumb', difficulty: 'Hard' },
  ...
  { answer: 'back to the drawing board', pattern: '4 2 3 7 5', visual: 'BOARD  ⟲\n✎  DRAW', description: 'An architect steps backwards from a failed model toward a drawing board to restart', difficulty: 'Hard' },
  { answer: 'off the charts', pattern: '3 3 6', visual: '│        ●\n│    ╱╱╱ ↑\n└──────────', description: 'A plotted value shoots beyond the top boundary of its chart', difficulty: 'Hard' },
  ...
  { answer: 'clear as mud', pattern: '5 2 3', visual: 'CLEAR', description: 'A transparent glass panel is almost completely obscured by thick mud', difficulty: 'Hard' },
  { answer: 'show your true colours', pattern: '4 4 4 7', visual: 'TRUE\nC O L O U R S', description: 'A person removes a plain grey mask to reveal a brilliantly coloured face', difficulty: 'Hard' },
  ...
  { answer: 'the ball is in your court', pattern: '3 4 2 2 4 5', visual: '', description: 'From the player’s viewpoint, a tennis ball rests inside their side of the court', format: 'illustration', difficulty: 'Hard' },
  ...
  { answer: 'the home stretch', pattern: '3 4 7', visual: 'H   O   M   E', description: 'A runner approaches a house whose roof and walls are stretched to an enormous length', difficulty: 'Hard' },
  { answer: 'hit home', pattern: '3 4', visual: 'HO  HIT  ME', description: 'A dart hits the exact centre of a target painted on a house', difficulty: 'Hard' },
  { answer: 'home away from home', pattern: '4 4 4 4', visual: 'HOME      AWAY      HOME', description: 'Two matching homes sit far apart on opposite sides of a globe', difficulty: 'Hard' },
  ...
  { answer: 'open doors', pattern: '4 5', visual: 'DO     ORS', description: 'Several ornate doors stand fully open along an inviting corridor', difficulty: 'Hard' },
  ...
  { answer: 'from the bottom of your heart', pattern: '4 3 6 2 4 5', visual: '   ♥\n  ↑\n╲  ●  ╱\n ╲___╱', description: 'A message rises from the deepest bottom point of a heart', difficulty: 'Hard' },
  { answer: 'heavy heart', pattern: '5 5', visual: '  ███\n  ███\n   ♥\n  ↓↓↓', description: 'A heart weighs down one side of a balance scale against light feathers', difficulty: 'Hard' },
  ...
  { answer: 'tickled pink', pattern: '7 4', visual: '〰  PINK  〰', description: 'A rosy-pink person laughs on the floor while being tickled', difficulty: 'Hard' },
  ...
  { answer: 'black mark', pattern: '5 4', visual: '┌──────────┐\n│    ●╲    │\n│   ╱██    │\n└──────────┘', description: 'A clean record is spoiled by one conspicuous black ink mark', difficulty: 'Hard' },
  ...
  { answer: 'hit the roof', pattern: '3 3 4', visual: '      HIT\n     ↑↑↑\n   ╱────╲', description: 'An angry person erupts upward and physically cracks the roof above', difficulty: 'Hard' },
  { answer: 'break new ground', pattern: '5 3 6', visual: 'GRO  BREAK  UND', description: 'A ceremonial shovel makes the first break in pristine untouched ground', difficulty: 'Hard' },
  { answer: 'stand your ground', pattern: '5 4 6', visual: '   STAND\n════════ GROUND ════════', description: 'A determined person stands firmly while roots anchor their boots into the ground', difficulty: 'Hard' },
  { answer: 'common ground', pattern: '6 6', visual: 'YOU      ME\n━━━ COMMON ━━━', description: 'Two opposing groups meet on one shared central patch of ground', difficulty: 'Hard' },
...
const chapterEightSpecs: ChapterFiveSpec[] = [
  ...
  { answer: 'back against the wall', pattern: '4 7 3 4', visual: 'BACK▐████████', description: 'A person stands with their back pressed flat against a brick wall', difficulty: 'Hard' },
  { answer: 'hit a brick wall', pattern: '3 1 5 4', visual: 'HIT  →  ▦▦▦▦\n          ▦▦▦▦', description: 'A runner collides directly with a solid brick wall', difficulty: 'Hard' },
  { answer: 'off the wall', pattern: '3 3 4', visual: '████ WALL ████     OFF', description: 'A framed painting floats completely away from the wall where it hung', difficulty: 'Hard' },
  { answer: 'drive someone up the wall', pattern: '5 7 2 3 4', visual: '      SOMEONE ↑\n      ↑       ▦\nDRIVE ↑       ▦ WALL', description: 'A car drives vertically up a wall while carrying a person upward', difficulty: 'Hard' },
  { answer: 'the walls have ears', pattern: '3 5 4 4', visual: '👂  ▦▦▦▦▦▦▦  👂\n    ▦  ▦  ▦', description: 'Two large ears grow directly from a brick wall', difficulty: 'Hard' },
  ...
  { answer: 'famous last words', pattern: '6 4 5', visual: '★  ★  ★                 WORDS │', description: 'The final WORDS tile in a long row sits alone on an illuminated gold star', difficulty: 'Hard' },
  ...
  { answer: 'building blocks', pattern: '8 6', visual: '   BLOCK\n BLOCK BLOCK\nBLOCK BLOCK BLOCK', description: 'Stacked wooden blocks progressively transform into a miniature building', difficulty: 'Hard' },
  { answer: 'connect the dots', pattern: '7 3 4', visual: '•──•──•──•──•', description: 'Every dot is joined into one connected path', difficulty: 'Hard' },
  { answer: 'piece together the puzzle', pattern: '5 8 3 6', visual: '╭PUZ╮  →←  ╭ZLE╮\n╰──凸╯      ╰凹──╯', description: 'Two interlocking puzzle pieces move toward one another to complete PUZZLE', difficulty: 'Hard' },
  ...
  { answer: 'ask for the moon', pattern: '3 3 3 4', visual: '( ? )  · · · · ·  🌙\n  🙋', description: 'An outstretched request reaches toward the impossibly distant moon', difficulty: 'Hard' },
  ...
  { answer: 'square the circle', pattern: '6 3 6', visual: '┌──────┐\n│  ◯   │\n└──────┘', description: 'A rigid square has been forced around a perfect circle', difficulty: 'Hard' },
  ...
  { answer: 'hit the ground running', pattern: '3 3 6 7', visual: '    RUNNING ↓\n══════ GROUND ══════', description: 'A runner hits the ground from a leap while already accelerating into a sprint', difficulty: 'Hard' },
  { answer: 'running out of steam', pattern: '7 3 2 5', visual: 'RUNNING       〰 〰 〰 →', description: 'An exhausted runner’s empty steam backpack releases its final tiny wisps', difficulty: 'Hard' },
  ...
  { answer: 'penny pincher', pattern: '5 7', visual: '〉〉  PENNY  〈〈', description: 'Two fingers tightly pinch a single penny', difficulty: 'Hard' },
  { answer: 'cost a fortune', pattern: '4 1 7', visual: '┌ $9,999,999 ┐\n└─────┬──────┘\n   💎💰👑', description: 'A treasure-sized fortune carries a price tag marked with double dollar signs and FORTUNE', difficulty: 'Hard' },
...
const chapterNineSpecs: ChapterFiveSpec[] = [
  ...
  { answer: 'when the chips are down', pattern: '4 3 5 3 4', visual: '◉ ◉ ◉\n ╲│╱\n  ▼\n DOWN', description: 'Four potato chips engraved W H E N sit along the absolute bottom of the scene', difficulty: 'Hard' },
  ...
  { answer: 'pins and needles', pattern: '4 3 7', visual: '●────▶   ───────▷   ●────▶\n   ───────▷', description: 'Sharp pins and sewing needles alternate in one prickly row', difficulty: 'Hard' },
  ...
  { answer: 'pin your hopes on something', pattern: '3 4 5 2 9', visual: '┌─────────────┐\n│ 📌 HOPES    │\n│  SOMETHING  │\n└─────────────┘', description: 'A hopeful note is physically pinned onto a larger board', difficulty: 'Hard' },
  { answer: 'pin down', pattern: '3 4', visual: 'PIN\n ↓\nDOWN', description: 'PIN is directed straight down', difficulty: 'Hard' },
  { answer: 'safety in numbers', pattern: '6 2 7', visual: '╭────────────╮\n│ 1 2 3 4 5  │\n╰───🛡🛡────╯', description: 'A close group of numbers shelters securely behind shields', difficulty: 'Hard' },
  { answer: 'number one priority', pattern: '6 3 8', visual: 'PRIORITY\n    1\n  ↑↑↑', description: 'A card labelled PRIORITY occupies the gold number-one position on a ranked board', difficulty: 'Hard' },
  ...
  { answer: 'your number is up', pattern: '4 6 2 2', visual: '      NUMBER\n        ↑\n        UP', description: 'A queue ticket number matches the same number illuminated on an overhead display', difficulty: 'Hard' },
  ...
  { answer: 'raise eyebrows', pattern: '5 8', visual: 'EYEBROWS\n ↑  ↑  ↑', description: 'A surprised person’s eyebrows repeatedly rise and settle', difficulty: 'Hard' },
  ...
  { answer: 'bad hair day', pattern: '3 4 3', visual: '〰╱╲〰╲╱〰╱╲〰\n┌──────────┐\n│   DAY    │\n└──────────┘', description: 'Wild tangled hair erupts from the top of a calendar day', difficulty: 'Hard' },
  { answer: 'by the skin of your teeth', pattern: '2 3 4 2 4 5', visual: 'T E E T H\n   skin', description: 'An impossibly thin pink skin-like layer stretches over a set of teeth', difficulty: 'Hard' },
  ...
  { answer: 'face value', pattern: '4 5', visual: '🏷  [ FACE ]  $', description: 'A coin showing a face sits beside a price tag marked with double dollar signs', difficulty: 'Hard' },
  ...
  { answer: 'under lock and key', pattern: '5 4 3 3', visual: '🔒       🔑\n   UNDER', description: 'A sealed envelope lies physically underneath both a lock and its key', difficulty: 'Hard' },
  ...
  { answer: 'a different kettle of fish', pattern: '1 9 6 2 4', visual: '♨  ♨  ♨     🫖\n             🐟', description: 'One unusual kettle contains a fish while all the others are ordinary', difficulty: 'Hard' },
...
const chapterTenSpecs: ChapterFiveSpec[] = [
  ...
  { answer: 'cast a wide net', pattern: '4 1 4 3', visual: 'CAST  →  ╱╲╱╲╱╲╱╲╱╲', description: 'A fisherman casts an enormous net across nearly the entire sea', difficulty: 'Hard' },
  ...
  { answer: 'a cog in the machine', pattern: '1 3 2 3 7', visual: 'MA  ⚙ COG  CHINE', description: 'One highlighted cog sits inside a complete industrial machine of pistons, belts and rollers', difficulty: 'Hard' },
  { answer: 'set the wheels in motion', pattern: '3 3 6 2 6', visual: 'SET  →  ⚙  ⚙  →→', description: 'A hand turns the first of several connected real wheels and starts them spinning', difficulty: 'Hard' },
  { answer: 'reinvent the wheel', pattern: '8 3 5', visual: '◉  →  ⚙  →  ◉\n   ✎ NEW ✎', description: 'A familiar wheel is redesigned through gears and a fresh blueprint', difficulty: 'Hard' },
  ...
  { answer: 'full steam ahead', pattern: '4 5 5', visual: '☁☁☁  🚂 ═════▶\n          AHEAD', description: 'A locomotive surrounded by full clouds of steam drives directly ahead', difficulty: 'Hard' },
]
